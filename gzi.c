...
static void gzi_cmd_apply_patch(gzi_ctxt_t *ctxt, int pos){
    ...
    switch(code.len){
        case 1:
            *((uint8_t*)(ctxt->file_ptrs[curfile] + code.offset)) = code.data & 0xFF;
        break;
        case 2:
            *((uint16_t*)(ctxt->file_ptrs[curfile] + code.offset)) = code.data & 0xFFFF;
        case 4:
        default:
            *((uint32_t*)(ctxt->file_ptrs[curfile] + code.offset)) = code.data;
            break;
    }
}
